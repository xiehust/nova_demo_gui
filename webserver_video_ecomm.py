...
####################  商品主图image。#############################
def create_production_images(user_prompt,reference_image,production):
    logger.info("商品主图图像生成")
    global system_division_2
    shots = get_divison_shots(system_division_2,user_prompt)
    prompts = [ f"{p['prompt']} {p['composition']} angle:{p['angle']} {p['distance']} {p['lighting']} {' '.join(p['style_tags'])}" for p in shots['shots']]
    neg_prompts = [p['negative_prompt'] for p in shots['shots']]
    ...
    #    logger.info(f"prompt:{prompt}\nneg_prompt:{neg_prompt}")
    #    save_path = os.path.join(output_dir,f'{timestamp}_shot_{idx}.png')
    #    generate_imageBgReplace(prompt,reference_image,production,save_path)
    #    image_files.append(save_path)
    #    time.sleep(2)
    ###只生成1张图
    logger.info(f"prompt:{prompts[0]}\n")
    save_path = os.path.join(output_dir,f'{timestamp}_shot_0.png')
    generate_imageBgReplace(prompts[0],reference_image,production,save_path)
    image_files.append(save_path)
    return image_files
...
def video_type(video_type_radio):
    ...
    if video_type_radio == "分镜视频":
        video_type = "divison"
    elif video_type_radio == "商品主图视频":
        video_type = "ecomm"
        gr.Info("请上传商品参考图，并在提示词中包含该商品的描述信息")
    else:
        video_type = "sequnce"
    # 清空 gallery_output
    return []
...
def generate_image(video_type_radio, user_prompt,reference_image=None):
    ...
    if video_type_radio == "商品主图视频" :
        if reference_image is None:
            gr.Info("请上传商品参考图片")
            return None
        else:
            user_prompt_en = _prompt_enhance(user_prompt)
            production = get_production_main(user_prompt_en)
            return create_production_images(user_prompt,reference_image,production)
    
    elif video_type_radio == '连续视频':
...
def create_video_with_task_id(image_paths,gradio_video_type,video_task_id):
    ...
    elif gradio_video_type == "商品主图视频":
        gr.Info("商品主图视频生成会依次生成，请耐心等待")
        return create_sequence_video(image_paths,video_task_id)
    else:
    ...
    with gr.Column():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                reference_image = gr.Image(label="参考图片", type="filepath")
        ...
        with gr.Row():
            with gr.Column():
                ...
                video_type_radio = gr.Radio(["分镜视频", "连续视频","商品主图视频"], label="生成视频类型",value="分镜视频")
        ...
        with gr.Row():
            with gr.Column(scale=1):
                generate_qr_btn = gr.Button("生成二维码")
            with gr.Column(scale=2):
                qr_output = gr.Image(label="视频下载二维码",type="numpy")
        
        # Connect the inputs and output
        generate_btn.click(
            fn=generate_image,
            #fn=create_divison_images,
            inputs=[video_type_radio,text_input,reference_image],
