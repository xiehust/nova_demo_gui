...
##################### 分镜头 image #############################
def create_divison_images(user_prompt,video_type_radio,num_shot):
    logger.info("分镜图像生成")
    global system_division_2
    global system_division_3
    shots = get_divison_shots(system_division_3.replace("<num_shot>",str(num_shot)),user_prompt)
    # prompts = [ f"{p['prompt']} {p['composition']} angle:{p['angle']} {p['distance']} {p['lighting']} {' '.join(p['style_tags'])}" for p in shots['shots']]
    prompts = [ f"{p['prompt']}" for p in shots['shots']]
    neg_prompts = [p['negative_prompt'] for p in shots['shots']]
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    output_dir=os.path.join('shot_images',timestamp)
    os.makedirs(output_dir, exist_ok=True)
...
def create_division_video(gradio_image_files,video_task_id=None):
    ...
    for p,ref_img in zip(shots['shots'],image_files):
        ...
        text = f"{p['prompt']}  {p['cinematography']}"
    ...
    caption_video_file = os.path.join(os.path.dirname(generated_fname),video_task_id+"_caption.mp4")
...
def generate_image(video_type_radio, user_prompt,num_shot):
...
def generate_qr_code(gradio_video_file):
    ...
    try:
        ...
        # 上传到AWS S3
        if not upload_to_s3(video_path, RESULT_QR_BUCKET, s3_file):
            return "Failed to upload video to S3."        
        # 生成S3 URL
        s3_url = generate_s3_url(RESULT_QR_BUCKET, s3_file)       
        # 生成二维码
        qr = qrcode.QRCode(
            version=1,
...
def generate_image_qr_code(selected_image_file):
    if not selected_image_file:
        gr.Info("请先生成图片")
        return None
    try:
        logger.info("image file is: " + selected_image_file)
        image_path = selected_image_file
        s3_file = image_path.split('/')[-1]
        
        # 上传到AWS S3
        if not upload_to_s3(image_path, RESULT_QR_BUCKET, s3_file):
    ...
    with gr.Column():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                num_shot = gr.Number(label="图片数量", value=2, minimum=1, maximum=5, step=1)
        ...
        with gr.Row():
            with gr.Column():
                ...
                video_type_radio = gr.Radio(["分镜视频", "连续视频"], label="生成视频类型",value="分镜视频")

        with gr.Row():
            with gr.Column(scale=2):
                video_task_id = gr.Textbox(label="我的视频名", interactive=True)
        ...
        with gr.Row():
            with gr.Column(scale=1):
                generate_qr_btn = gr.Button("生成视频下载二维码")
                generate_image_qr_btn = gr.Button("生成图片下载二维码") 
            with gr.Column(scale=2):
                qr_output = gr.Image(label="下载二维码",type="numpy")
        
        selected_image_file = gr.Textbox(visible=False)
        
        # Connect the inputs and output
        generate_btn.click(
            fn=generate_image,
            #fn=create_divison_images,
            inputs=[video_type_radio,text_input,num_shot],
        ...
        generate_image_qr_btn.click(
            ...
            inputs=selected_image_file,  # 使用selected_image_file作为输入
            outputs=[qr_output],
            concurrency_limit=8
        )
        ...
        gallery_output.select(
            ...
            outputs=selected_image_file
        )

# Launch the app
if __name__ == "__main__":
    demo.queue(max_size=30).launch(server_name="0.0.0.0", share=True,server_port=7861)